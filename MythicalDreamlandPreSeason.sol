...
contract MythicalDreamlandPreSeason {
    ...
    constructor(address _inhibAddress) {
        ...
        setInhibitorAddress(_inhibAddress);
    }
    ...
    function stakeUnits(address _stakingContract, bool _optIntoLock) public callerIsUser {
        ...
        if (!stakeableUnits[location].stakeableContract.getStakingStatus(_msgSender()) && stakeableUnits[location].stakeableContract.balanceOf(_msgSender()) > 1){
    ...
    function unstakeUnits(address _stakingContract) public callerIsUser {
        uint256 location = _stakeableUnitsLocations[_stakingContract];
        require(location != 0, "UnknownStakeableUnit");
        require(stakerData[_msgSender()].stakedUnlockTimestamp == 0 || stakerData[_msgSender()].stakedUnlockTimestamp < block.timestamp, "OptionalLockPeriodActive");
        aggregateRewards(_msgSender());
    ...
    function aggregateRewards(address _address) public {
        require (_approvedUnits[_msgSender()] || _address == _msgSender(), "CallerLacksAggregationPermissions");
    ...
    function needsRewardsUpdate(address _address) public view returns (bool) {
        return 600 < getTimeSinceClaimed(_address);//change back to 3600
    }
     ...
     */
    function setInhibitorAddress(address _address) public onlyOwner {
     ...
     */
    function setLordAddress(address _address) public onlyOwner {
